...
var Lien = module.exports = function (opt_options) {
    ...
    var requestHandler = function (request, response) {
        ...
        var handler = self.ruut && self.ruut(request.url)
        if (handler) {
            var l = LienCreator.call(self, request, response)
            l.params = handler.params;
            handler.data(l);
            return;
        }
    ...
    // Handle ssl
    if (opt_options.certificates) {
        opt_options.certificates._key = opt_options.certificates._key || Fs.readFileSync(opt_options.certificates.key);
        opt_options.certificates._cert = opt_options.certificates._cert || Fs.readFileSync(opt_options.certificates.cert);
    }

    // Create HTTP server
    self._server = opt_options.certificates
                 ? Https.createServer({
                       key: opt_options.certificates._key
                     , cert: opt_options.certificates._cert
                   }, requestHandler)
    ...
    self.page.add = function (route, file, method, callback) {
        ...
        if (typeof route === "string" && ~route.indexOf(":")) {
            var splits = route.split("/").filter(Boolean);
            var pRoute = self._params_routes;
            splits.forEach(function (cSplit, i) {
                if (i === splits.length - 1) {
                    pRoute[cSplit] = function (data) {
                        return file;
                    };
                }
                pRoute = (pRoute[cSplit] = pRoute[cSplit] || {});
            });
            self.ruut = Ruut(self._params_routes);
            return;
        }
