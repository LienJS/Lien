...
var Lien = module.exports = function (opt_options) {
    ...
    var requestHandler = function (request, response) {
        ...
        var handler = self.ruut && self.ruut(parsed.pathname);
        if (handler) {
            var l = LienCreator.call(self, request, response, url)
    ...
    // Handle ssl
    if (opt_options.ssl) {
        opt_options.ssl._key = opt_options.ssl._key || Fs.readFileSync(opt_options.ssl.key);
        opt_options.ssl._cert = opt_options.ssl._cert || Fs.readFileSync(opt_options.ssl.cert);
    }

    // Create HTTP server
    self._server = opt_options.ssl
                 ? Https.createServer({
                       key: opt_options.ssl._key
                     , cert: opt_options.ssl._cert
                   }, requestHandler)
