...
class LienObj {
     ...
     */
        ...
        let trans = this.server.getHooks("after", this.method, this.path);
    ...
    _sendResponse (resData) {
        ...
        if (res.content && res.content.statusCode) {
            res.status = res.content.statusCode;
        }
...
module.exports = class Lien extends EventEmitter {
    ...
    addPage (url, method, output) {
        ...
        this.router[method](url, (req, res) => {
            ...
            let trans = this.server.getHooks("before", req.method.toLowerCase(), req.path);

            if (trans) {
                let lien = new LienObj(req, res, this);
                trans.start((err, data) => {
    ...
    hook (where, url, method, cb, transType) {
        ...
        if (method === "all") {
            httpMethods.forEach(c => this.hook(where, url, c, cb));
            return this;
        }
        ...
        switch (where) {
            case "before":
            case "after":
                trans = this.getHooks(where, method, url) || this.insertHook(where, method, url, new Transformer());
                break;
            default:
