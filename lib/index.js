 ...
 *    - `engine` (String): The view engine name.
 *
 *  - `errorPages` (Object):
...
module.exports = class Lien extends EventEmitter {
    constructor (options) {
        ...
        if (options.template) {
            app.set("views", options.template.views);
            app.set("view engine", options.template.engine);
        }
    ...
    _handleRoute (req, res, output, args) {
        let lien = new LienObj(req, res, noop, this);
